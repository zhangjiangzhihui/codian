...
export default class ClaudianPlugin extends Plugin {
  ...
  normalizeModelVariantSettings(): boolean {
    if (this.settings.agentProvider !== 'claude') {
      return false;
    }
  ...
  private getDefaultModelValues(): string[] {
    if (this.settings.agentProvider === 'codex') {
      return getCodexModelOptions(this.settings.customCodexModels, this.settings.model).map((m) => m.value);
    }
  ...
  private computeEnvHash(envText: string): string {
    ...
    const modelKeys = this.settings.agentProvider === 'codex'
      ? []
      : [
        'ANTHROPIC_MODEL',
    ...
    const providerKeys = this.settings.agentProvider === 'codex'
      ? ['OPENAI_BASE_URL', 'CODEX_API_KEY', 'OPENAI_API_KEY']
      : ['ANTHROPIC_BASE_URL'];
    ...
    const relevantPairs = allKeys
      ...
      .sort()
      .join('|');
    return `${this.settings.agentProvider}|${relevantPairs}`;
  }
  ...
  private reconcileModelWithEnvironment(envText: string): {
    ...
    if (this.settings.agentProvider === 'claude') {
      ...
      this.settings.model = DEFAULT_CODEX_MODEL;
    }
