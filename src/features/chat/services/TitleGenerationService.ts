    ...
    if (this.plugin.settings.agentProvider === 'codex') {
      ...
      try {
        const codex = new Codex({
          apiKey: envVars.CODEX_API_KEY || envVars.OPENAI_API_KEY || undefined,
          baseUrl: envVars.OPENAI_BASE_URL || undefined,
          codexPathOverride: resolvedCodexPath,
          env: {
            ...process.env,
            ...envVars,
            PATH: getEnhancedPath(envVars.PATH, resolvedCodexPath),
          } as Record<string, string>,
        });

        const titleModel = this.plugin.settings.titleGenerationModel || this.plugin.settings.model;
