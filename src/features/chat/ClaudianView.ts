...
import { BRAND_SVGS } from './constants';
...
export class ClaudianView extends ItemView {
  ...
  refreshBranding(): void {
    ...
    if (this.logoEl) {
      this.renderLogo(this.logoEl, branding.provider);
    }

    if (this.titleTextEl) {
      this.titleTextEl.setText(branding.title);
    }
  ...
  private buildHeader(header: HTMLElement) {
    ...
    this.renderLogo(this.logoEl, this.getBranding().provider);

    // Title text (hidden in header mode when 2+ tabs)
    this.titleTextEl = this.titleSlotEl.createEl('h4', { text: this.getBranding().title, cls: 'claudian-title-text' });
  ...
  private getBranding(): { provider: 'claude' | 'codex'; title: string } {
    const provider = this.plugin.settings.agentProvider === 'codex' ? 'codex' : 'claude';
    return {
      provider,
  ...
  private renderLogo(container: HTMLElement, provider: 'claude' | 'codex'): void {
    container.empty();

    const logo = BRAND_SVGS[provider];
    ...
    for (const shapeDef of logo.shapes) {
      if (shapeDef.type === 'circle') {
        const circle = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        circle.setAttribute('cx', shapeDef.cx);
        circle.setAttribute('cy', shapeDef.cy);
        circle.setAttribute('r', shapeDef.r);
        if (shapeDef.fill) circle.setAttribute('fill', shapeDef.fill);
        const stroke = 'stroke' in shapeDef ? shapeDef.stroke : undefined;
        const strokeWidth = 'strokeWidth' in shapeDef ? shapeDef.strokeWidth : undefined;
        if (typeof stroke === 'string') circle.setAttribute('stroke', stroke);
        if (typeof strokeWidth === 'string') circle.setAttribute('stroke-width', strokeWidth);
      ...
      if (shapeDef.fill) path.setAttribute('fill', shapeDef.fill);
      const stroke = 'stroke' in shapeDef ? shapeDef.stroke : undefined;
      const strokeWidth = 'strokeWidth' in shapeDef ? shapeDef.strokeWidth : undefined;
      const transform = 'transform' in shapeDef ? shapeDef.transform : undefined;
      if (typeof stroke === 'string') path.setAttribute('stroke', stroke);
      if (typeof strokeWidth === 'string') path.setAttribute('stroke-width', strokeWidth);
      if (typeof transform === 'string') path.setAttribute('transform', transform);
